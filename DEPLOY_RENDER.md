...
5. **Where would you like to connect from?** → choose "My Local Environment" → **Add a Different IP Address** → enter `0.0.0.0/0` → Description: `allow all` → confirm
...
✅ Save this connection string — you'll paste it into Render in step 4.

---

## Step 2: Save the project to GitHub (2 min)
...
## Step 4: Deploy via Blueprint (5 min)
...
4. Render will prompt you for the two **secret** env vars:
   ...
   - `EMERGENT_LLM_KEY` → paste from `/app/backend/.env` (the `sk-emergent-...` value)
5. Click **Apply** → Render starts building (takes ~5–10 minutes for the first build)

> ☁️ The free tier spins down after 15 min of inactivity. First request after sleep takes ~30 seconds. For always-on (no cold start), upgrade to **Starter** ($7/mo) in `render.yaml` → `plan: starter`.
...
## Step 6: Update the APK to use your Render backend (3 min)
...
### Why `EMERGENT_LLM_KEY` still works outside Emergent
The key is a hosted service operated by Emergent — your backend just makes HTTPS calls to their endpoint. It works from anywhere. As long as the key has balance, your AI avatar generator keeps working. Top up balance anytime via the Emergent UI: Profile → Universal Key → Add Balance.
...
- Keeps the service warm 24/7 (technically against Render's spirit-of-free-tier, but works)

### CORS
Already handled — your `server.py` has `allow_origins=["*"]`, so the APK can call Render from any device.

### Switching to fully self-hosted AI (optional, advanced)
If you ever want to drop `emergentintegrations`:
1. Get a free Google Gemini API key at https://aistudio.google.com
2. Replace the `LlmChat` block in `server.py` with the official `google-generativeai` SDK
3. Set `GOOGLE_API_KEY` env var in Render instead of `EMERGENT_LLM_KEY`
...
| Problem | Fix |
|---|---|
| Build fails: `Could not find a version that satisfies emergentintegrations` | Verify `render.yaml` has the `--extra-index-url https://d33sy5i8bnduwe.cloudfront.net/simple/` flag in `buildCommand` |
